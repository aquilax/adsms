 ...
 */
  ...
  function insertMobio($post){
    ...
    /*
fromnum: този параметър съдържа номера на мобилния телефон на Вашия
     * потребител. Този параметър ще трябва да използвате при изпращането на
     * SMS съобщение с отговор до Вашия потребител;
message: пълният текст на SMS-а, изпратен от Вашия потребител;
item: това е числото в съдържанието на SMS съобщението след Вашата
     * ключова дума. Например, ако потребител изпрати SMS съобщение paypn112, item
     * ще бъде 112'
extid: това е уникален номер за проверка от системата на Mobio. Нужно е да
     * изпратите стойността на този параметър без промяна при заявката за
     * изпращане на SMS'
